...
**Cross-cutting gap found while fixing that**: `restore.sh.j2` only
restores raw files/volumes - it does not re-import a `.sql` dump into a
running Postgres automatically. This isn't unique to mail-archiver
(umami, forgejo, nocturne all use the same pg_dump-to-`backup_paths`
pattern) - it's a repo-wide gap. Restoring any Postgres-backed service
today means: run `restore.sh --service <name>` to get the `.sql` file
and volumes back on disk, then manually `docker exec <db-container> psql
... < dump.sql` (or equivalent) to actually load it. Worth fixing
generically at some point (a `restore_post_hook` mirroring the existing
`backup_post_hook` pattern), not done as part of this audit - flagging
...
- Fix the generic Postgres-restore gap (`restore.sh.j2` doesn't
  re-import `.sql` dumps automatically) - affects every pg_dump-backed
  service, not just mail-archiver.
