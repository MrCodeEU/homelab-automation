...
// KumaDate parses either RFC3339 or Uptime Kuma's plain
// "YYYY-MM-DD HH:MM:SS" datetime format. Kuma 2.5.x sends
// Maintenance.DateRange in the plain format (no "T"/offset), which the
// standard library's time.Time JSON unmarshaling (RFC3339-only) rejects -
// this vendored fork exists solely to fix that mismatch. See
// tools/thirdparty/go-uptime-kuma-client/PATCHES.md.
type KumaDate time.Time

func (d *KumaDate) UnmarshalJSON(data []byte) error {
	var s string
	if err := json.Unmarshal(data, &s); err != nil {
		return err
	}
	if s == "" || s == "null" {
		*d = KumaDate(time.Time{})
		return nil
	}
...
func (d KumaDate) MarshalJSON() ([]byte, error) {
...
type Maintenance struct {
	...
	DateRange []*KumaDate `json:"dateRange"`
